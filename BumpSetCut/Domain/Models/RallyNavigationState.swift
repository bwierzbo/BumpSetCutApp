...
final class RallyNavigationState: ObservableObject {
    ...
    // MARK: - Smart Preloading State
    @Published var preloadingStatus: PreloadingStatus = .idle
    @Published var preloadedPlayerIndex: Int? = nil
    @Published var currentPlayerSlot: PlayerSlot = .primary
    ...
    // MARK: - Smart Preloading Management
    @MainActor
    func triggerPreloading(for targetIndex: Int) {
        guard let metadata = processingMetadata,
              targetIndex >= 0 && targetIndex < metadata.rallySegments.count,
              targetIndex != currentRallyIndex,
              preloadingStatus == .idle else {
            return
        }

        preloadingStatus = .loading
        preloadedPlayerIndex = targetIndex
        preloadingProgress = 0.0

        print("RallyNavigationState: Starting preload for rally \(targetIndex + 1)")
    }

    @MainActor
    func updatePreloadingProgress(_ progress: Double) {
        preloadingProgress = min(1.0, max(0.0, progress))
    }

    @MainActor
    func completePreloading(success: Bool) {
        if success {
            preloadingStatus = .ready
            preloadingProgress = 1.0
            print("RallyNavigationState: Preloading completed for rally \(preloadedPlayerIndex?.advanced(by: 1) ?? 0)")
        } else {
            preloadingStatus = .failed
            preloadedPlayerIndex = nil
            print("RallyNavigationState: Preloading failed")
        }
    }

    @MainActor
    func swapToPreloadedPlayer() -> Bool {
        guard preloadingStatus == .ready,
              let preloadedIndex = preloadedPlayerIndex else {
            return false
        }

        // Swap players: the preloaded becomes current
        currentPlayerSlot = currentPlayerSlot == .primary ? .secondary : .primary
        preloadingStatus = .idle
        preloadedPlayerIndex = nil
        preloadingProgress = 0.0

        print("RallyNavigationState: Swapped to preloaded player (slot: \(currentPlayerSlot))")
        return true
    }

    @MainActor
    func shouldPreload(targetIndex: Int) -> Bool {
        // Preload next rally if we're not already preloading and target is different
        return preloadingStatus == .idle &&
               targetIndex != currentRallyIndex &&
               targetIndex >= 0 &&
               (processingMetadata?.rallySegments.count ?? 0) > targetIndex
    }

    @MainActor
    func getNextPreloadTarget() -> Int? {
        // Always try to preload the next rally for 1-ahead strategy
        let nextIndex = currentRallyIndex + 1
        guard let metadata = processingMetadata,
              nextIndex < metadata.rallySegments.count else {
            return nil
        }
        return nextIndex
    }
    ...
    func reset() {
        ...
        // Reset smart preloading state
        preloadingStatus = .idle
        preloadedPlayerIndex = nil
        currentPlayerSlot = .primary
        preloadingProgress = 0.0
    }
}
