...
final class FrameExtractor {
    ...
    func extractFrame(from videoURL: URL, at time: CMTime? = nil, priority: ExtractionPriority = .normal) async throws -> UIImage {
        let frameTime = time ?? config.frameTime
        let extractionStartTime = Date()

        // Create cache key that includes the time (so different times from same video are cached separately)
        let cacheKey = createCacheKey(url: videoURL, time: frameTime)

        // Update telemetry
        telemetry.totalExtractions += 1
        ...
        if memoryMonitor.isUnderMemoryPressure {
            ...
            if memoryMonitor.currentMemoryPressure.contains(.critical) {
                telemetry.errorEvents += 1
                throw FrameExtractionError.memoryPressure
            }
        }

        // Check cache first (using time-aware cache key)
        if let cachedImage = cache.get(cacheKey) {
            telemetry.cacheHits += 1
            logger.debug("✅ Cache hit for frame extraction: \(videoURL.lastPathComponent) at \(frameTime.seconds)s")
            return cachedImage
        }
        ...
        let image = try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<UIImage, Error>) in
            selectedQueue.async { [weak self] in
                ...
                let timeoutTask = DispatchWorkItem { [weak self] in
                    ...
                    Task { @MainActor in
                        self?.telemetry.timeoutEvents += 1
                        self?.logger.error("⏰ Frame extraction timeout for: \(videoURL.lastPathComponent)")
                    }
                    resumeOnce(with: .failure(FrameExtractionError.timeoutExceeded))
                }

                DispatchQueue.global().asyncAfter(deadline: .now() + timeout, execute: timeoutTask)

                // Extract frame at specified time
                let requestedTime = frameTime
                imageGenerator.generateCGImagesAsynchronously(forTimes: [NSValue(time: requestedTime)]) { [weak self] (requestedTime, cgImage, actualTime, result, error) in
                    timeoutTask.cancel()

                    if let error = error {
                        Task { @MainActor in
                            self?.telemetry.errorEvents += 1
                            self?.logger.error("❌ AVFoundation error: \(error.localizedDescription)")
                        }
                        resumeOnce(with: .failure(FrameExtractionError.avFoundationError(error)))
                        return
                    }
                    ...
                    guard result == .succeeded, let cgImage = cgImage else {
                        ...
                        resumeOnce(with: .failure(FrameExtractionError.imageGenerationFailed))
                        return
                    }

                    let uiImage = UIImage(cgImage: cgImage)
                    resumeOnce(with: .success(uiImage))
                }
            }
        }
        ...
        telemetry.totalExtractionTime += extractionTime

        logger.debug("⏱️ Frame extraction completed in \(Int(extractionTime * 1000))ms for: \(videoURL.lastPathComponent)")

        // Cache the extracted image if not under severe memory pressure (using time-aware key)
        if !memoryMonitor.currentMemoryPressure.contains(.critical) {
            await MainActor.run {
    ...
    /// Creates a cache key URL that includes the extraction time
    /// This ensures frames at different times from the same video are cached separately
    private func createCacheKey(url: URL, time: CMTime) -> URL {
        var components = URLComponents(url: url, resolvingAgainstBaseURL: false)
        // Append time to fragment to create unique cache key
        let timeFragment = "t_\(String(format: "%.3f", time.seconds))"
        if let existingFragment = components?.fragment {
            components?.fragment = "\(existingFragment)_\(timeFragment)"
        } else {
            components?.fragment = timeFragment
        }
