...
struct ParabolicValidator {
    ...
    func validateTrajectory(_ positions: [(CGPoint, CMTime)]) -> ValidationResult {
        ...
        guard let fitResult = fitQuadratic(points: timeBasedPoints) else {
            return ValidationResult(
                isValid: false,
                r2Correlation: 0.0,
                curvatureDirection: .invalid,
    ...
    func calculateR2Correlation(_ positions: [(CGPoint, CMTime)]) -> Double {
        guard let firstTime = positions.first?.1 else { return 0.0 }
        let baseTime = CMTimeGetSeconds(firstTime)
        let timeBasedPoints = positions.map { pos -> CGPoint in
            let t = CMTimeGetSeconds(pos.1) - baseTime
            return CGPoint(x: t, y: pos.0.y)
        }
        return fitQuadratic(points: timeBasedPoints)?.r2 ?? 0.0
    }
