...
final class DebugVideoExporter: ObservableObject {
    ...
    private func createTrackedBallFromTrajectories(
        ...
        guard let firstPoint = recentPoints.first else { return nil }

        // Create a tracked ball with the first position, then update with remaining
        let config = ProcessorConfig()
        var trackedBall = KalmanBallTracker.TrackedBall(
            position: firstPoint.0,
            timestamp: firstPoint.1,
            config: config,
            maxPositions: 1000
        )
        for point in recentPoints.dropFirst() {
            trackedBall.update(measurement: point.0, timestamp: point.1, config: config)
        }
        return trackedBall
    }
}
