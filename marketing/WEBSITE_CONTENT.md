# BumpSetCut - Website Content Guide

Complete content for your marketing website including copy, features, FAQ, and contact information.

---

## Hero Section

### Headline
**Never Miss a Rally Again**

### Subheadline
Automatically detect, extract, and share volleyball rallies from your videos. BumpSetCut uses AI to find the action so you can focus on the game.

### CTA Buttons
- **Download on the App Store** (primary)
- **Watch Demo Video** (secondary)

### Hero Image/Video
- Show app interface with video processing in action
- Or: Demo video showing before/after (full game video → extracted rallies)
- Alternative: Split screen showing phone with app + volleyball action

---

## Features Section

### Section Title
**Smart Volleyball Analysis**

### Feature 1: AI-Powered Rally Detection
**Icon:** 🎯 (or brain/target icon)

**Title:** Automatic Rally Detection

**Description:**
Our computer vision AI watches your videos and automatically identifies when the ball is in play. No more scrubbing through hours of footage—BumpSetCut finds every rally for you.

**Details:**
- CoreML-powered volleyball detection
- Tracks ball trajectory with physics validation
- Distinguishes between beach and indoor volleyball
- Processes videos up to 4K resolution

### Feature 2: Full-Screen Rally Viewer
**Icon:** 📱 (or swipe gesture icon)

**Title:** Swipe Through Your Best Moments

**Description:**
Review your rallies with a smooth, vertical swipe interface. Built for volleyball highlights. Perfect for quick review sessions after practice or games.

**Details:**
- Vertical swipe navigation
- Landscape and portrait support
- Slow-motion playback controls
- Frame-by-frame scrubbing

### Feature 3: Smart Organization
**Icon:** 📁 (or folder tree icon)

**Title:** Organize Your Volleyball Library

**Description:**
Create custom folders for tournaments, practices, or players. Tag videos by sport type and search your entire library instantly.

**Details:**
- Custom folder organization
- Beach vs indoor auto-detection
- Search by filename, date, or tag
- Quick filters for processed vs original videos

### Feature 4: Social Sharing
**Icon:** 🌐 (or share icon)

**Title:** Share Highlights with Your Team

**Description:**
Post your best rallies to the BumpSetCut community. Follow teammates, comment on plays, and build your volleyball highlight reel.

**Details:**
- Public highlight feed
- Follow players and teams
- Like and comment on rallies
- User profiles with highlight collections

### Feature 5: Export & Save
**Icon:** 💾 (or download icon)

**Title:** Export to Camera Roll

**Description:**
Save processed rallies directly to your photo library or share them anywhere. Your highlights, your way.

**Details:**
- Export individual rallies
- Save to Photos app
- Share to social media
- Optional debug overlay for coaches

### Feature 6: Pro Features
**Icon:** ⭐ (or crown icon)

**Title:** Unlock Unlimited Processing

**Description:**
Go Pro for unlimited video processing, no watermarks, and cellular uploads. Perfect for serious players and coaches.

**Details:**
- Unlimited video processing
- No watermarks on exports
- Upload on cellular data
- Priority processing queue

---

## How It Works

### Section Title
**From Game Video to Highlight Reel in Minutes**

### Step 1
**📹 Import Your Video**
Record a match or upload existing footage from your camera roll. BumpSetCut supports videos up to 500MB (free) or unlimited (Pro).

### Step 2
**🤖 AI Processes Automatically**
Our computer vision AI scans every frame, tracking the volleyball and detecting when rallies start and end. Physics validation ensures accurate detection.

### Step 3
**🎬 Review & Share**
Swipe through extracted rallies in seconds. Export your favorites to camera roll or share directly to the community feed.

---

## Pricing Section

### Section Title
**Start Free. Upgrade When Ready.**

### Free Plan
**$0**

**Perfect for casual players**

Features:
- ✅ 3 videos per week
- ✅ Videos up to 500MB
- ✅ WiFi-only uploads
- ✅ AI rally detection
- ✅ Full-screen rally viewer
- ✅ Export with watermark
- ✅ Community sharing
- ✅ Custom folders

CTA: **Download Free**

### Pro Plan
**$7.99/month**

**For serious players & coaches**

Features:
- ✅ **Unlimited videos**
- ✅ **No file size limits**
- ✅ **Cellular uploads**
- ✅ **No watermarks**
- ✅ AI rally detection
- ✅ Full-screen rally viewer
- ✅ Export to camera roll
- ✅ Community sharing
- ✅ Custom folders
- ✅ Priority processing

CTA: **Start Free Trial**

*Cancel anytime. No commitment.*

---

## Social Proof / Testimonials

### Section Title
**Trusted by Players and Coaches**

### Testimonial 1
*"BumpSetCut saved me hours of video review. I can now focus on analyzing plays instead of searching for them."*
— **Sarah M.**, Beach Volleyball Coach

### Testimonial 2
*"The AI detection is surprisingly accurate. It catches rallies I would have missed scrolling through manually."*
— **Jake T.**, Club Player

### Testimonial 3
*"Love the swipe-through interface. Makes reviewing game footage actually fun instead of a chore."*
— **Emily R.**, College Athlete

*(Note: Replace with real testimonials once you have beta testers)*

---

## FAQ Section

### General Questions

#### What is BumpSetCut?
BumpSetCut is an iOS app that uses AI to automatically detect and extract volleyball rallies from your game videos. Instead of watching hours of footage, our computer vision technology finds the action for you.

#### How accurate is the rally detection?
Our CoreML model is trained specifically on volleyball footage and uses physics validation to ensure accurate rally boundaries. Detection accuracy is typically 90%+ for well-lit outdoor and indoor courts.

#### What video formats are supported?
BumpSetCut supports all standard iOS video formats (MOV, MP4, HEVC). Videos can be imported from your camera roll or recorded directly in the app (coming soon).

#### Does it work for both beach and indoor volleyball?
Yes! BumpSetCut automatically detects whether you're playing beach or indoor volleyball and adjusts detection accordingly.

#### Can I use it for practice footage?
Absolutely. BumpSetCut works on any volleyball video—matches, practices, drills, or scrimmages. The AI detects ball movement, not game context.

### Technical Questions

#### What devices are supported?
BumpSetCut requires iOS 17.0 or later. It works on iPhone 12 and newer models. iPad support coming soon.

#### Does it work offline?
Yes! Video processing happens entirely on your device using CoreML. No internet connection required for rally detection. Internet is only needed for uploading videos and accessing the community feed.

#### How long does processing take?
Processing time depends on video length and device. Typically, a 30-minute video processes in 5-10 minutes on an iPhone 14 or newer. Pro users get priority processing.

#### Where are my videos stored?
All videos are stored locally on your device. Processed rallies can be exported to your camera roll. If you share highlights to the community, they're hosted securely on Supabase cloud storage.

#### Does it drain my battery?
Video processing is CPU-intensive and will use battery. We recommend processing videos while charging. Once processed, viewing rallies uses minimal battery.

### Account & Billing

#### Is there a free trial?
Yes! The free plan includes 3 video processing credits per week with no time limit. Try it as long as you want.

#### What happens if I cancel my Pro subscription?
You keep all processed videos on your device. You'll revert to the free plan limits (3 videos/week, watermarks). Your community highlights remain public.

#### Can I get a refund?
Subscriptions are managed through the App Store. You can cancel anytime within 24 hours of purchase for a full refund. Contact Apple Support for refund requests.

#### Is there a team or coach discount?
Not yet, but we're working on team plans. Contact us at support@bumpsetcut.com if you're interested in bulk licensing.

### Privacy & Content

#### Who can see my videos?
Videos you process stay private on your device unless you choose to share highlights to the community feed. You control what gets shared.

#### Can I delete my account?
Yes. Go to Settings → Account → Delete Account. This removes all your data from our servers. Locally stored videos remain on your device until you delete the app.

#### How do you handle inappropriate content?
Users can report posts for spam, harassment, or inappropriate content. We review all reports within 24 hours. See our [Community Guidelines](https://bumpsetcut.com/community-guidelines) for details.

#### Do you sell my data?
Never. We don't sell, rent, or share your personal information with third parties. See our [Privacy Policy](https://bumpsetcut.com/privacy) for full details.

### Troubleshooting

#### Why isn't my video processing?
Common issues:
- **Video too large:** Free users have a 500MB limit
- **Low storage:** Processing requires 2x the original video size in free space
- **Unsupported format:** Use MOV or MP4 formats
- **Weekly limit reached:** Free users can process 3 videos per week

#### The AI missed some rallies. Why?
Rally detection works best with:
- Clear view of the court
- Good lighting (avoid heavy shadows)
- Ball visible in frame
- Minimal camera movement

If detection is inaccurate, try recording from a higher angle or with better lighting.

#### App crashes during processing. Help!
Try these steps:
1. Restart the app
2. Ensure you have 5GB+ free storage
3. Close other apps to free up RAM
4. Update to the latest iOS version
5. If issues persist, contact support@bumpsetcut.com

#### Can I re-process a video?
Currently, videos can only be processed once to prevent duplicate rallies. If you want to re-process with different settings, delete the original and re-import.

---

## CTA Section (Bottom of Page)

### Section Title
**Ready to Level Up Your Volleyball Game?**

### Description
Join thousands of players using BumpSetCut to analyze, share, and improve their game. Start free today.

### CTA Buttons
- **Download on iOS** (primary)
- **Watch Demo** (secondary)

---

## Footer

### Company
**BumpSetCut**
Automatic volleyball rally detection for players and coaches.

### Links

**Product**
- Features
- Pricing
- How It Works
- Download

**Resources**
- FAQ
- Privacy Policy
- Terms of Service
- Community Guidelines

**Support**
- Contact Us
- Report a Bug
- Feature Requests

**Community**
- Instagram: @bumpsetcut
- Twitter/X: @bumpsetcut
- TikTok: @bumpsetcut
- Discord: Join Server

### Contact

**Email:**
- General: hello@bumpsetcut.com
- Support: support@bumpsetcut.com
- Privacy: privacy@bumpsetcut.com
- Press: press@bumpsetcut.com

**Mailing Address:**
BumpSetCut
[Your Business Address]
[City, State ZIP]

### Legal
© 2024 BumpSetCut. All rights reserved.

[Privacy Policy](https://bumpsetcut.com/privacy) | [Terms of Service](https://bumpsetcut.com/terms) | [Community Guidelines](https://bumpsetcut.com/community-guidelines)

---

## Design Notes

### Brand Colors
- **Primary:** Volleyball orange (#FF6B35) or court blue (#4A90E2)
- **Secondary:** Deep navy (#1A2332) or black
- **Accent:** Bright green (#00C896) for success states
- **Background:** Clean white or light gray (#F8F9FA)

### Typography
- **Headlines:** Bold, modern sans-serif (Inter, SF Pro Display, Montserrat)
- **Body:** Clean, readable sans-serif (Inter, SF Pro Text, Open Sans)
- **Accents:** Optional sporty font for "BumpSetCut" logo

### Imagery
- High-quality volleyball action shots
- Phone mockups showing app interface
- Before/after video comparisons
- Players using the app courtside

### Tone of Voice
- **Energetic but professional**
- **Technical but accessible** (explain AI without jargon)
- **Action-oriented** (focus on saving time, improving game)
- **Community-focused** (emphasize sharing and connection)

---

## SEO Keywords

### Primary Keywords
- volleyball video analysis app
- automatic rally detection
- volleyball highlight creator
- AI volleyball coach app
- volleyball video editor

### Secondary Keywords
- beach volleyball app
- indoor volleyball training
- volleyball game film
- volleyball practice video
- volleyball clip maker
- volleyball social network

### Long-Tail Keywords
- how to extract volleyball rallies from video
- app that finds volleyball plays automatically
- best volleyball video analysis app for iPhone
- volleyball highlight reel maker
- AI-powered volleyball detection

---

## Call-to-Action Variations

Use these CTAs throughout the site:

**Primary CTAs:**
- Download on the App Store
- Start Free Today
- Get BumpSetCut Free
- Try It Free

**Secondary CTAs:**
- Watch Demo Video
- See How It Works
- Learn More
- View Pricing

**Conversion CTAs:**
- Upgrade to Pro
- Start Your Free Trial
- Go Unlimited
- Remove Watermarks

---

## Social Media Assets

### Instagram Bio
📱 AI-powered volleyball rally detection
🎯 Never miss a moment
⬇️ Download for iOS
[Link to App Store]

### Twitter/X Bio
Automatic volleyball rally detection for players & coaches. AI finds the action so you can focus on the game. Download for iOS 👇

### Short-Form Video Bio
🏐 AI finds your best rallies
📱 Download for iOS
👇 Link below

---

## Press Kit Content

### Boilerplate
BumpSetCut is an iOS application that uses artificial intelligence and computer vision to automatically detect and extract volleyball rallies from game footage. Founded in 2024, BumpSetCut helps players, coaches, and teams save hours of video review time by instantly identifying the moments that matter. The app features AI-powered detection, a full-screen rally viewer, custom organization, and social sharing capabilities.

### Key Facts
- **Launch Date:** [Your Launch Date]
- **Platform:** iOS 17.0+
- **Price:** Free with Pro upgrade ($7.99/month)
- **Technology:** CoreML, SwiftUI, AVFoundation, Supabase
- **Target Users:** Volleyball players, coaches, teams, and enthusiasts

### Founder Quote
*"I created BumpSetCut because I was tired of spending hours searching through game footage to find specific plays. As both a player and a developer, I knew AI could solve this problem. Now anyone can analyze their game film in minutes instead of hours."*
— [Your Name], Founder

---

## Demo Video Script (60 seconds)

**[0-5s] Hook**
*Visual: Hours of game footage fast-forwarding*
"Tired of scrubbing through hours of volleyball footage?"

**[5-10s] Problem**
*Visual: Person frustrated, scrolling timeline*
"Finding rallies manually wastes time and kills momentum."

**[10-20s] Solution**
*Visual: Phone with BumpSetCut logo, app opening*
"BumpSetCut uses AI to find every rally automatically. Just import your video..."

**[20-35s] Demo**
*Visual: App processing video, progress bar*
"...and our computer vision scans every frame, tracking the ball and detecting rallies."

*Visual: Swipe through rallies in full-screen viewer*
"Swipe through your highlights in seconds."

**[35-50s] Features**
*Visual: Show features quickly*
- "Organize by tournament"
- "Share with your team"
- "Export to camera roll"

**[50-60s] CTA**
*Visual: App Store download button*
"Start free today. 3 videos per week, no credit card required. Download BumpSetCut now."

*Visual: Logo + App Store badge*

---

## Email Templates

### Welcome Email (New User)

**Subject:** Welcome to BumpSetCut! 🏐 Let's find your best rallies

Hi [Name],

Welcome to BumpSetCut! You're now part of a community of volleyball players using AI to level up their game analysis.

**Here's how to get started:**

1️⃣ **Import your first video**
Tap the + button and choose a video from your camera roll. We'll process it automatically.

2️⃣ **Review your rallies**
Swipe through extracted rallies in a full-screen viewer. Slow-motion and frame-by-frame controls included.

3️⃣ **Share your best moments**
Export to camera roll or share to the BumpSetCut community feed.

**Free Plan Includes:**
✅ 3 videos per week
✅ AI rally detection
✅ Full-screen rally viewer
✅ Export & share

Need more? Upgrade to Pro for unlimited processing and no watermarks.

Questions? Reply to this email or visit our [FAQ](https://bumpsetcut.com/faq).

Let's get started!

The BumpSetCut Team

---

### Pro Upgrade Email

**Subject:** Unlock unlimited rallies with BumpSetCut Pro ⭐

Hi [Name],

We noticed you've reached your weekly processing limit. You're clearly getting value from BumpSetCut! 🎉

**Upgrade to Pro and unlock:**
- ✅ Unlimited video processing
- ✅ No file size limits
- ✅ Upload on cellular data
- ✅ No watermarks on exports
- ✅ Priority processing queue

**Just $7.99/month. Cancel anytime.**

Ready to go unlimited?

[Upgrade to Pro]

Not ready yet? Your free credits reset every Monday.

Thanks for using BumpSetCut!

The BumpSetCut Team

P.S. Questions about Pro? Check our [pricing FAQ](https://bumpsetcut.com/faq#billing).

---

## Content Checklist

Use this checklist when building your website:

**Essential Pages:**
- [ ] Home (hero, features, how it works, pricing, CTA)
- [ ] FAQ
- [ ] Privacy Policy (already created)
- [ ] Terms of Service (already created)
- [ ] Community Guidelines (needs to be created)
- [ ] Contact page

**Recommended Pages:**
- [ ] About / Team
- [ ] Demo video
- [ ] Blog (for SEO and updates)
- [ ] Press kit
- [ ] Careers (if hiring)

**Assets Needed:**
- [ ] App icon (high-res, transparent background)
- [ ] Screenshots (iPhone 16 Pro size recommended)
- [ ] Demo video (30-60 seconds)
- [ ] Feature graphics/icons
- [ ] App Store badge (official Apple asset)

**Technical Setup:**
- [ ] Domain: bumpsetcut.com
- [ ] SSL certificate (HTTPS)
- [ ] Analytics (Google Analytics, Plausible, or Fathom)
- [ ] SEO meta tags (title, description, Open Graph)
- [ ] Favicon
- [ ] robots.txt and sitemap.xml

**Pre-Launch:**
- [ ] Test all links
- [ ] Mobile responsive design
- [ ] Load time optimization
- [ ] Accessibility check (WCAG AA)
- [ ] Privacy policy links working
- [ ] App Store link working (when available)

---

## Hosting Recommendations

### Option 1: Static Site Generators (Recommended)
**Tools:** Next.js, Astro, Hugo, Jekyll
**Hosting:** Vercel, Netlify, GitHub Pages (all free)
**Pros:** Fast, SEO-friendly, easy to maintain
**Best for:** Clean, professional marketing site

### Option 2: No-Code Builders
**Tools:** Webflow, Framer, Carrd
**Pros:** Visual design, no coding needed
**Best for:** Quick launch, design-focused founders

### Option 3: WordPress
**Hosting:** WordPress.com, Bluehost, SiteGround
**Pros:** Mature ecosystem, easy blog
**Best for:** If you want a blog and CMS

### Option 4: Simple HTML/CSS
**Hosting:** Netlify Drop (drag-and-drop), GitHub Pages
**Pros:** Complete control, lightweight
**Best for:** If you're comfortable coding HTML

**My Recommendation:** Use Vercel + Next.js for free hosting with great performance. Or Carrd ($19/year) if you want something up in an hour.

---

## Launch Checklist

**Week Before Launch:**
- [ ] Website live and tested
- [ ] App Store listing submitted and approved
- [ ] Privacy policy, terms, guidelines hosted
- [ ] Social media accounts created
- [ ] Email addresses set up (hello@, support@, etc.)
- [ ] Demo video rendered and uploaded
- [ ] Press kit prepared

**Launch Day:**
- [ ] App goes live on App Store
- [ ] Share on social media
- [ ] Email launch announcement (if you have a list)
- [ ] Post in volleyball communities (Reddit, Facebook groups)
- [ ] Submit to Product Hunt (optional)
- [ ] Reach out to volleyball blogs/influencers

**Week After Launch:**
- [ ] Monitor app reviews and respond
- [ ] Track analytics (downloads, website visits)
- [ ] Gather user feedback
- [ ] Fix critical bugs if any
- [ ] Plan first update

---

## Questions to Answer Before Launch

1. **What's your business structure?** (LLC, sole proprietor, etc.)
2. **Do you have a business address** for the footer and App Store listing?
3. **What's your domain?** (bumpsetcut.com, getbumpsetcut.com, etc.)
4. **Do you have email set up?** (hello@, support@, privacy@)
5. **Do you have demo footage?** (gameplay video you can use)
6. **Do you have beta testers** for testimonials?
7. **What's your launch date target?**
8. **Do you have a logo/brand identity?**

---

That's everything you need! Let me know if you want me to expand any section or create additional assets (community guidelines, demo video storyboard, social media templates, etc.).
